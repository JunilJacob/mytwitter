...
def mainpage(request):
	if not request.user.is_authenticated():
		...
		try:
			...
			html = "ok"#t.render(Context({ 'user' : request.user,'entries': None}))
		else:
	...
	if request.method == 'POST':
		...
		if uname and upass:
			try:
				...
				user.save()
				user = Follow(username = uname,followers = "",following = uname)
				user.save()
				user = auth.authenticate(username=uname, password=upass)
				if user is not None and user.is_active:
					auth.login(request, user)
...
def newpost(request):
	if request.method == 'POST':
		...
		user = request.user
		to = re.findall(r'@([^\s@]+)',data)
		if not to:
				to=[request.user]
		try:
			p = Follow.objects.get(username=to[0])
		except Follow.DoesNotExist:
			return HttpResponse("error")
		else:
			data= re.sub(r'(@[^\s@]+)', r'', data)
			p = Post(username = user, post = data,touser = to[0])
			p.save()
			return HttpResponseRedirect("/")
def follow(request):
	if request.method == 'POST':
		...
		try:
			...
			p.save()
			p = Follow.objects.get(username=followuser)
			p.followers=p.followers	+ " " +	str(request.user)
			p.save()
			return HttpResponseRedirect("/")
